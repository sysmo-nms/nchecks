...
public class CheckIfXMulticast implements CheckInterface {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CheckIfXMulticast.class);
    private static final String IF_INDEX = "1.3.6.1.2.1.2.2.1.1";
    private static final String IF_IN_MULTICAST = "1.3.6.1.2.1.31.1.1.1.2";
    private static final String IF_OUT_MULTICAST = "1.3.6.1.2.1.31.1.1.1.4";

    private static final OID[] COLUMNS = new OID[]{
        new OID(IF_INDEX),
        new OID(IF_IN_MULTICAST),
        new OID(IF_OUT_MULTICAST)
    };

    public CheckIfXMulticast() {
    }
    ...
    public Reply execute(Query query) {
        ...
        try {
            ...
            CheckIfXMulticast.LOGGER.error(e.getMessage(), e);
        ...
        try {
            ...
            List<TableEvent> snmpReply = tableWalker.getTable(
                    target, CheckIfXMulticast.COLUMNS,
            ...
            if (newStatus.equals(Status.OK)) {
                replyMsg = "CheckIfXMulticast OK";
            } else if (newStatus.equals(Status.UNKNOWN)) {
                replyMsg = "CheckIfXMulticast UNKNOWN. No enough data to set sensible status.";
            } else if (newStatus.equals(Status.WARNING)) {
                replyMsg = "CheckIfXMulticast WARNING have found errors!";
            } else if (newStatus.equals(Status.CRITICAL)) {
                replyMsg = "CheckIfXMulticast CRITICAL have found errors!";
            } else {
                replyMsg = "";
            }
