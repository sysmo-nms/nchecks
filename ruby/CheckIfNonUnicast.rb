...
$IF_OUT_NON_UNICAST = "1.3.6.1.2.1.2.2.1.28";
...
def check(query) # query is io.sysmo.nchecks.Query
  ...
  walker.addColumn($IF_IN_NON_UNICAST)
  walker.addColumn($IF_OUT_NON_UNICAST)
  ...
  snmp_reply.each { |event|
      ...
      if arg_if_selection_list.include?(Integer.toString(interface_index))
          nuIn  = variable_bindings[1].getVariable().toLong()
  ...
  begin
      ...
      if    new_status == Status::OK
          reply.setReply("CheckIfNonUnicast OK ")
      elsif new_status == Status::UNKNOWN
          reply.setReply("CheckIfNonUnicast UNKNOWN. No enough data to set sensible status.")
      elsif new_status == Status::WARNING
          reply.setReply("CheckIfNonUnicast WARNING have found errors!")
      elsif new_status == Status::CRITICAL
          reply.setReply("CheckIfNonUnicast CRITICAL have found errors!")
      end

  rescue Exception => e
      new_status = Status::ERROR
      reply.setReply("CheckIfNonUnicast: #{e.message}")
  end
