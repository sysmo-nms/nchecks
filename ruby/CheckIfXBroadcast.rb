...
def check(query) # query is io.sysmo.nchecks.Query
  ...
  walker.addColumn($IF_IN_BROADCAST)
  walker.addColumn($IF_OUT_BROADCAST)
  ...
  snmp_reply.each { |event|
      ...
      if arg_if_selection_list.include?(Integer.toString(interface_index))
          bcIn  = variable_bindings[1].getVariable().toLong()
          bcOut = variable_bindings[2].getVariable().toLong()
          reply.putPerformance(interface_index, "IfInBroadcastPkts", bcIn)
          reply.putPerformance(interface_index, "IfOutBroadcastPkts",  bcOut)
  ...
  begin
      ...
      if    new_status == Status::OK
          reply.setReply("CheckIfXBroadcast OK ")
      elsif new_status == Status::UNKNOWN
          reply.setReply("CheckIfXBroadcast UNKNOWN. No enough data to set sensible status.")
      elsif new_status == Status::WARNING
          reply.setReply("CheckIfXBroadcast WARNING have found errors!")
      elsif new_status == Status::CRITICAL
          reply.setReply("CheckIfXBroadcast CRITICAL have found errors!")
      end

  rescue Exception => e
      new_status = Status::ERROR
      reply.setReply("CheckIfXBroadcast: #{e.message}")
  end
